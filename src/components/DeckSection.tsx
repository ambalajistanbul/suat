  ...
  return (
    <div className="glass rounded-3xl shadow-2xl overflow-hidden">
      <div className="grid grid-cols-1 lg:grid-cols-12 gap-0 divide-y lg:divide-y-0 lg:divide-x divide-white/10">
        ...
        {/* Slide Canvas Cover */}
        <div className="lg:col-span-6 relative aspect-video lg:aspect-auto min-h-[300px] lg:min-h-[480px] bg-slate-950 group">
          <img 
            src={slide.image} 
          ...
          {/* Deck page index */}
          <div className="absolute top-6 left-6 bg-slate-950/90 border border-white/10 px-3 py-1.5 rounded-xl font-mono text-[10px] text-brand-gold font-bold">
            {t.slideOf} · {currentSlideIndex + 1} / {DECK_SLIDES.length}
          </div>

          <div className="absolute bottom-6 left-6 right-6">
            <h4 className="text-xl md:text-2xl font-black font-display text-white tracking-tight drop-shadow-md leading-none">
              {slideContent.title}
            </h4>
            <span className="text-xs md:text-base font-semibold text-brand-gold/90 font-mono tracking-wide mt-1 block">
        ...
        {/* Slide Texts & Details */}
        <div className="lg:col-span-6 p-6 md:p-8 flex flex-col justify-between space-y-6 bg-slate-950/40">
            ...
            <AnimatePresence mode="wait">
              ...
              >
                {slideContent.paragraphs.map((p, idx) => (
                  <p key={idx} className="text-slate-300 text-sm leading-relaxed font-normal">
                    {p}
                  </p>
                ))}
          ...
          <div className="flex items-center justify-between border-t border-white/5 pt-6">
              ...
              {DECK_SLIDES.map((_, idx) => (
                <button
                  ...
                  className={`w-2 h-2 rounded-full cursor-pointer transition-all duration-300 ${
                    currentSlideIndex === idx ? 'w-6 bg-brand-blue' : 'bg-slate-800 hover:bg-slate-700'
                  }`}
            ...
            <div className="flex items-center gap-2">
              <button
                onClick={handlePrev}
                className="w-10 h-10 border border-white/10 hover:bg-white/5 text-slate-300 rounded-xl flex items-center justify-center cursor-pointer transition-colors"
              ...
              <button
                onClick={handleNext}
                className="w-24 h-10 btn-primary text-white font-semibold text-xs rounded-xl flex items-center justify-center gap-1 cursor-pointer transition-colors"
